...
export interface MediaFile {
  ...
  size: number;
  /**
   * The local url of the file, including the name.
   */
  localURL: string;
  /**
...
export class MediaCaptureMock extends MediaCapture {
   ...
   * @param options
   * @returns {Promise<MediaFile|any[]>}
   */
  captureVideo(options?: CaptureVideoOptions): Promise<MediaFile[] | CaptureError> {
    const mediaFileMock: MediaFile = {
      name: 'VID_20180528_123145.mp4',
      type: 'video/mp4',
      lastModifiedDate: new Date(1527503508000),
      size: 2308654,
      fullPath: 'assets/Camera/VID_20180528_123145.mp4',
      localURL: 'file:///assets/Camera/VID_20180528_123145.mp4',
      getFormatData() {
        return {
          name: this.name,
          type: this.type,
    ...
    let response: Array<MediaFile> = [mediaFileMock];
    return new Promise((resolve, reject) => {
